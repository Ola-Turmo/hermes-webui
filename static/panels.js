...
function _cronOutputSnippet(content) {
  ...
  // If there is no response section, do not dump the whole prompt into the sidebar.
  const lines = content.split('\n');
  const responseIdx = lines.findIndex(l => l.startsWith('## Response') || l.startsWith('# Response'));
  if (responseIdx < 0) {
    const failed = lines.some(l => /FAILED/i.test(l));
    return failed ? '(run failed before a response was written)' : '(no response section recorded yet)';
  }
  const body = lines.slice(responseIdx + 1).join('\n').trim();
