...
(async()=>{
  ...
  // Restore last-used model preference, but discard stale legacy GPT defaults when the server default is MiniMax.
  const savedModel=localStorage.getItem('hermes-webui-model');
  const serverDefault=(_bootSettings&&_bootSettings.default_model)?_bootSettings.default_model:null;
  if(savedModel && $('modelSelect')){
    const legacyStaleModels=['openai/gpt-5.4-mini','gpt-5.4-mini','GPT-5.4 Mini'];
    if(serverDefault && legacyStaleModels.includes(savedModel) && savedModel!==serverDefault){
      localStorage.removeItem('hermes-webui-model');
      $('modelSelect').value=serverDefault;
    } else {
  ...
  } else if(serverDefault && $('modelSelect')) {
    $('modelSelect').value=serverDefault;
  }
