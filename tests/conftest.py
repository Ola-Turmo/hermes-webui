...
def test_server():
    ...
    # NOTE: TEST_STATE_DIR.mkdir() already created a real 'skills' dir, so we
    # must remove it before symlinking.  exists() is True for real dirs, so
    # we use is_symlink() to detect whether the symlink was already made.
    real_skills  = HERMES_HOME / 'skills'
    test_skills  = TEST_STATE_DIR / 'skills'
    if real_skills.exists() and not test_skills.is_symlink():
        if test_skills.is_dir() and not test_skills.is_symlink():
            test_skills.rmdir()          # remove empty real dir
        test_skills.symlink_to(real_skills, target_is_directory=True)

    # Isolated cron state
    (TEST_STATE_DIR / 'cron').mkdir(parents=True, exist_ok=True)

    # NOTE: Do NOT set HERMES_HOME to TEST_STATE_DIR.
    # pathlib.Path.home() respects $HOME, and TEST_STATE_DIR is a throwaway
    # temp dir whose parent (/root/.hermes) is where the real config.yaml
    # lives.  Setting HERMES_HOME=/tmp/... would make Path.home() return /tmp,
    # causing Session.workspace to default to /tmp instead of TEST_WORKSPACE.
    # HERMES_WEBUI_AGENT_DIR is sufficient for agent discovery.
    ...
    env.update({
        ...
        # HERMES_HOME intentionally omitted — see NOTE above.
    })
